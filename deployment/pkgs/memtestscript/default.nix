...
writeScriptBin name ''
  ...
  testMemAlert()
  {
      ...
      # become the specified available percentage value.
      # The third data point is deliberately outside the time window to force
      # the alert node to evaluate the mean value.

      ${influxCmd} -execute "INSERT mem,host=test2 available_percent=$active $(convertToNS $offset)"
      offset=$((offset + $window))
      ${influxCmd} -execute "INSERT mem,host=test2 available_percent=$active $(convertToNS $offset)"
      offset=$((offset + $window))
      ${influxCmd} -execute "INSERT mem,host=test2 available_percent=$active $(convertToNS $offset)" # deliberately outside the time window
      offset=$((offset + $window))
  ...
  testMemAlert 10 60 critical
  testMemAlert 30 60 warning
  testMemAlert 100 60 ok
''
